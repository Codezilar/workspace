...
export async function POST(req) {
  ...
  try {
    ...
    const job = await Job.findOne({ _id: data.jobId, status: "ACTIVE" });
    if (!job)
      return NextResponse.json(
        { error: "This role is not available" },
        { status: 404 },
      );
    const application = await Application.create({ ...data, userId: user._id });
...
export async function GET() {
  ...
  await connectDB();
  return NextResponse.json(
    await Application.find(isAdmin(u) ? {} : { userId: u._id })
